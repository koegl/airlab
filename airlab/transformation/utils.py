...
def upsample_displacement(displacement, new_size, interpolation="linear"):
    ...
    if dim == 2:
        displacement = th.transpose(
            displacement.unsqueeze(0), 0, 3).unsqueeze(0)
        ...
        displacement = th.transpose(
            displacement.unsqueeze(0), 0, 4).unsqueeze(0)
        if interpolation == 'linear':
...
def get_displacement_itk(displacement, refIm):
    ...
    dispIm = sitk.GetImageFromArray(
        ...
        .transpose(list(range(dim-1, -1, -1)) + [dim])[..., ::-1],
        isVector=True
    )
...
def rotation_matrix(phi_x, phi_y, phi_z, dtype=th.float32, device='cpu', homogene=False):
    R_x = th.Tensor([[1, 0, 0], [0, th.cos(phi_x), -th.sin(phi_x)],
                    [0, th.sin(phi_x), th.cos(phi_x)]])
    R_y = th.Tensor([[th.cos(phi_y), 0, th.sin(phi_y)], [
                    0, 1, 0], [-th.sin(phi_y), 0, th.cos(phi_y)]])
    R_z = th.Tensor([[th.cos(phi_z), -th.sin(phi_z), 0],
                    [th.sin(phi_z), th.cos(phi_z), 0], [0, 0, 1]])
...
class Diffeomorphic():
    ...
    def diffeomorphic_2D(displacement, grid, scaling=-1):
        ...
        displacement = displacement.transpose(
            2, 1).transpose(1, 0).unsqueeze(0)

        for i in range(scaling):
            displacement_trans = displacement.transpose(1, 2).transpose(2, 3)
            displacement = displacement + \
                F.grid_sample(displacement, displacement_trans + grid)
    ...
    def diffeomorphic_3D(displacement, grid, scaling=-1):
        ...
        displacement = displacement.transpose(
            3, 2).transpose(2, 1).transpose(0, 1).unsqueeze(0)

        for i in range(scaling):
            displacement_trans = displacement.transpose(
